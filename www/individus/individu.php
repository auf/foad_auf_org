...
function formulaireIndividu($cnx, $tab, $confirm=0)
{
	$CIVILITE = array(
		"Madame",
		"Monsieur"
	...
	$form .= "<tr>\n<th>CNF&nbsp;:</th>\n<td>" ;
	$form .= listeCnf("cnf",
		( isset($tab["cnf"]) ? $tab["cnf"] : "" ),
		TRUE) ;
	$form .= "</td>\n</tr>\n" ;

	$form .= "<tr>\n<th>Civilité&nbsp;:</th>\n<td>" ;
	$form .= formSelect1($CIVILITE, "civilite",
		( isset($tab["civilite"]) ? $tab["civilite"] : "" ),
		TRUE) ;
	$form .= "</td>\n</tr>\n" ;

	$form .= "<tr>\n<th>Prénom&nbsp;:</th>\n<td>" ;
	$form .= '<input type="text" name="prenom" size="70" value="' ;
	$form .= ( isset($tab["prenom"]) ? $tab["prenom"] : "" ) ;
	$form .= '" />' ;
	$form .= "</td>\n</tr>\n" ;

	$form .= "<tr>\n<th>Nom&nbsp;:</th>\n<td>" ;
	$form .= '<input type="text" name="nom" size="70" value="' ;
	$form .= ( isset($tab["nom"]) ? $tab["nom"] : "" ) ;
	$form .= '" />' ;
	$form .= "</td>\n</tr>\n" ;

	$form .= "<tr>\n<th>Courriel&nbsp;:</th>\n<td>" ;
	$form .= '<input type="text" name="courriel" size="80" value="' ;
	$form .= ( isset($tab["courriel"]) ? $tab["courriel"] : "" ) ;
	$form .= '" />' ;
	$form .= "</td>\n</tr>\n" ;

	$form .= "<tr>\n<th>Actif&nbsp;:</th>\n<td>" ;
	$form .= radioActifInactif("actif",
		( isset($tab["actif"]) ? $tab["actif"] : "" )
		) ;
	if ( $confirm )
	{
		$form .= "<div class='erreur'>" ;
		$form .= "Rendre un individu inactif entraîne sa suppression des destinataires de tous les fils de messages." ;
		$form .= "<br />\n" ;
	...
	$form .= "<input type='hidden' name='id_individu' value='"
		. ( isset($tab["id_individu"]) ? $tab["id_individu"] : "" )
		. "' />\n" ;
...
if ( isset($tabIndividu) AND is_array($tabIndividu) ) {
...
if ( isset($_POST["submit"]) )
{
	...
	if ( $erreurs )
	{
		...
		echo $apresMenu ;
		echo $erreurs ;
		echo formulaireIndividu($cnx, $_POST) ;
	}
	else
	{
		if ( $id_individu != 0 )
		{
			if ( ($tabIndividu["actif"] == "1") AND ($_POST["actif"] == "0") )
			{
				...
				if ( ($N != "0") AND ($_POST["confirm"] != "confirm") )
				{
					...
					include("inc_menu.php") ;
					echo $apresMenu ;
					echo formulaireIndividu($cnx, $_POST, $N) ;
					exit() ;
					deconnecter($cnx) ;
					echo $end ;
				}
...
else
{
	if ( $id_individu != 0 )
	{
			...
			include("inc_menu.php") ;
			echo $apresMenu ;
			echo formulaireIndividu($cnx, $tabIndividu) ;
	}
	else
	{
		echo $avantMenu ;
		include("inc_menu.php") ;
		echo $apresMenu ;
		echo formulaireIndividu($cnx, array()) ;
	}
}
